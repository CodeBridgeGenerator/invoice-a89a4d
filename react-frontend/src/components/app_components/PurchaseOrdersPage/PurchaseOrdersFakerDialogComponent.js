...
import purchaseOrdersFakerFactory from "./purchaseOrdersFakerFactory";
...
const PurchaseOrdersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchaseOrdersFakerFactory(props.user,fakerCount,quotationIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchaseOrders").create(elem));
...
export default connect(mapState, mapDispatch)(PurchaseOrdersFakerDialogComponent);
