...
const ProductsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.productID)) {
                error["productID"] = `Product ID field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.productType)) {
                error["productType"] = `Product Type field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            productID: _entity?.productID,name: _entity?.name,description: _entity?.description,unitPrice: _entity?.unitPrice,productType: _entity?.productType,unitPrice: _entity?.unitPrice,
    ...
    return (
        <Dialog header="Create Products" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="productID" className="w-full mb-3 p-inputtext-sm" value={_entity?.productID} onChange={(e) => setValByKey("productID", e.target.value)}  required  />
            </span>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="productType" className="w-full mb-3 p-inputtext-sm" value={_entity?.productType} onChange={(e) => setValByKey("productType", e.target.value)}  required  />
            </span>
            <small className="p-error">
