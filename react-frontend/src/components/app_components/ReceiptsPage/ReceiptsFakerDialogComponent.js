...
const ReceiptsFakerDialogComponent = (props) => {
    ...
    const [receiptIDIds, setreceiptIDIds] = useState();
const [paymentIDIds, setpaymentIDIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("payments")
                        .find({ query: { $select: ["_id","receiptID"] } })
                        .then((data) => {
                        setreceiptIDIds(data.data);
                        });
client.service("payments")
    ...
    const onRun = async () => {
        let fakeData = receiptsFakerFactory(props.user,fakerCount,receiptIDIds,paymentIDIds);
