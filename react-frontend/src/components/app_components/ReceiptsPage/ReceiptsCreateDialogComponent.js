...
const [paymentID, setPaymentID] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [receiptID,paymentID], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            receiptID: _entity?.receiptID?._id,paymentID: _entity?.paymentID?._id,dateIssued: _entity?.dateIssued,receiptDetails: _entity?.receiptDetails,
    ...
    useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setReceiptID(res.data.map((e) => { return { name: e['receiptID'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
    ...
    const receiptIDOptions = receiptID.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Receipts" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <Dropdown id="receiptID" value={_entity?.receiptID?._id} optionLabel="name" optionValue="value" options={receiptIDOptions} onChange={(e) => setValByKey("receiptID", {_id : e.value})}  />
            </span>
