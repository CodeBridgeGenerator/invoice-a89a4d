...
const QuotationsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.quotationID)) {
                error["quotationID"] = `Quotation ID field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            quotationID: _entity?.quotationID,customerID: _entity?.customerID,quotationDate: _entity?.quotationDate,status: _entity?.status,totalAmount: _entity?.totalAmount,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Quotations created successfully" });
