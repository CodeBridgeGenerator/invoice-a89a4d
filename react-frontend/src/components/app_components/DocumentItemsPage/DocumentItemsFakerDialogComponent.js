...
import documentItemsFakerFactory from "./documentItemsFakerFactory";
...
const DocumentItemsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [documentItemsIds, setdocumentItemsIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = documentItemsFakerFactory(props.user,fakerCount,documentItemsIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("documentItems").create(elem));
...
export default connect(mapState, mapDispatch)(DocumentItemsFakerDialogComponent);
