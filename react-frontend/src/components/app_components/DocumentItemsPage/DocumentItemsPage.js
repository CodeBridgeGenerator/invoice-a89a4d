...
import DocumentItemsDatatable from "./DocumentItemsDataTable";
import DocumentItemsEditDialogComponent from "./DocumentItemsEditDialogComponent";
import DocumentItemsCreateDialogComponent from "./DocumentItemsCreateDialogComponent";
import DocumentItemsFakerDialogComponent from "./DocumentItemsFakerDialogComponent";
import DocumentItemsSeederDialogComponent from "./DocumentItemsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DocumentItemsPage = (props) => {
    ...
    const filename = "documentItems.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("documentItems");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , documentItems : urlParams.singleServicesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Document Items", type: "error", message: error.message || "Failed get Document Items" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("documentItems").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Document Items", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("documentItems").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/documentItems/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="documentItems-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="documentItems-datatable">
                    <DocumentItemsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <DocumentItemsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DocumentItemsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <DocumentItemsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DocumentItemsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DocumentItemsPage);
