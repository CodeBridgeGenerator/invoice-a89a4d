
    module.exports = function (app) {
        const modelName = 'services';
        const mongooseClient = app.get('mongooseClient');
        const { Schema } = mongooseClient;
        const schema = new Schema(
          {
            serviceID: { type:  String , required: true, minLength: null, maxLength: null },
name: { type:  String , minLength: null, maxLength: 150, index: true, trim: true },
description: { type:  String , required: true, minLength: null, maxLength: null },
units: { type:  String , required: true, minLength: null, maxLength: null },
serviceType: { type:  String , required: true, minLength: null, maxLength: null },
price: { type:  String , required: true, maxLength: null },

            
            createdBy: { type: Schema.Types.ObjectId, ref: "users", required: true },
            updatedBy: { type: Schema.Types.ObjectId, ref: "users", required: true }
          },
          {
            timestamps: true
        });
      
       
        if (mongooseClient.modelNames().includes(modelName)) {
          mongooseClient.deleteModel(modelName);
        }
        return mongooseClient.model(modelName, schema);
        
      };